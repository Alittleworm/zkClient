...
int32_t ZookeeperManager::InitFromConf(const char *config_file_path, const clientid_t *client_id)
{
	...
	TIniFile tIni;
	if (!tIni.Open(config_file_path))
	{
		return ZBADARGUMENTS;
	}

	string strIP = tIni.ReadString("main", "zkIP", "");
	if (strIP.empty())
	{
		return ZBADARGUMENTS;
	}

	m_tConf.m_strZKIP = strIP;
	m_tConf.m_strZKRoot = tIni.ReadString("main", "root", "/");
	if (m_tConf.m_strZKRoot.empty())
	{
		return ZBADARGUMENTS;
	}
	m_tConf.m_strLocalIP = tIni.ReadString("main", "localIP", "");
	if (m_tConf.m_strLocalIP.empty())
	{
		return ZBADARGUMENTS;
	}

	m_tConf.m_iRecvTimeout = tIni.ReadInt("main", "recvTime", 1000);
	m_tConf.m_iConnectTimeout = tIni.ReadInt("main", "conTime", 1000);
	if (m_tConf.m_iRecvTimeout < 1000)
	{
		m_tConf.m_iRecvTimeout = 1000;
	}
	if (m_tConf.m_iConnectTimeout < 1000)
	{
		m_tConf.m_iConnectTimeout = 1000;
	}

	return Init(m_tConf.m_strZKIP, m_tConf.m_strZKRoot, client_id);
}


// 获取配置
TZKConf &ZookeeperManager::GetConf()
{
	return m_tConf;
}
...
int32_t ZookeeperManager::ConnectByConf(std::shared_ptr<WatcherFunType> global_watcher_fun)
{
	return Connect(global_watcher_fun, m_tConf.m_iRecvTimeout, m_tConf.m_iConnectTimeout);
}
