...
public:
	...
	localIP=127.0.0.1
	*/
	int32_t InitFromConf(const char *config_file_path="../conf/zookeeper.ini", const clientid_t *client_id = NULL);
	...
	// 同上
	// recv_timeout_ms = m_tConf.m_iRecvTimeout
	// conn_timeout_ms = m_tConf.m_iConnecTimeout
	int32_t ConnectByConf(std::shared_ptr<WatcherFunType> global_watcher_fun);
